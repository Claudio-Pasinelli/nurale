    ...
    return(
        <Flex direction='row' alignItems='center' height='48.5px' borderBottomRightRadius='2rem' boxShadow='rgba(81, 70, 137, 0.3) 0px 2px 6px'>
            ...
            <Flex padding='0.5rem 1rem'>
                ...
                {
                    isProfileOpen ? 
                    <Flex width='18rem' padding='1.25rem 1.3rem' position='absolute' top='6.5%' right='0' backgroundColor={darkModePalette.main2} direction='column' boxShadow='rgba(81, 70, 137, 0.3) -0.2rem 0.2rem 0.4rem' borderRadius='0.8rem' zIndex={100000} marginRight='0.5rem' overflow='hidden'>
                        <Flex>
                            <Flex fontSize={theme.fontSizes.xs} fontWeight={theme.fontWeights.medium} width='2.5rem' height='2.5rem' alignItems='center' justifyContent='space-around' borderRadius='100%' backgroundColor={darkModePalette.pink100}>
                                <p>{`${firstName?.substring(0, 1)}${lastName?.substring(0, 1)}`}</p>
                            </Flex>
                            <Flex marginLeft='0.5rem' alignSelf='center' fontSize={theme.fontSizes.xxs} fontWeight={theme.fontWeights.bold}>
